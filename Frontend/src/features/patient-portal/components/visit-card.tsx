  ...
  return (
    <Link
      ...
      className={cn(
        ...
        highlighted ? "border-teal-200 bg-gradient-to-br from-teal-600 to-teal-700 text-white" : "border-slate-200/80 bg-white",
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <span className={cn("grid size-11 place-items-center rounded-2xl", highlighted ? "bg-white/15" : "bg-teal-50 text-teal-700")}>
      ...
      <div className={cn("mt-1.5 flex items-center justify-between gap-3 text-xs", highlighted ? "text-teal-50" : "text-slate-500")}>
        <span>Mã lượt: {shortVisitId(visit.id)}</span>
        <span>{formatDateTime(visit.createdAt)}</span>
      </div>
      <div className={cn("mt-4 flex items-center justify-between border-t pt-3 text-xs font-semibold", highlighted ? "border-white/20 text-white" : "border-slate-100 text-teal-700")}>
        Xem tiến trình
